...
import { steps, useRoutes } from "@/providers/OnboardingProvider";
import Image from "next/image";
import Link from "next/link";

export default function OnboardingNav() {
  const { isPreviousRoute, currentRoute } = useRoutes();

  return (
    <header className="container v-stack justify-center items-center gap-5 p-6 border-b-foreground">
        ...
        {steps.map((step) => {
          ...
          return (
            <Comp
              ...
              <span className={cn("w-full h-2 rounded-md bg-muted", active && "bg-primary")}></span>
            </Comp>
          );
        })}
      </nav>
